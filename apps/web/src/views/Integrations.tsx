...
export function Integrations() {
  ...
  const [mailBusy, setMailBusy] = useState<'test' | 'verify' | null>(null);
              ...
              <>
                ...
                <div className="border-t border-[#1f1f22] pt-5 space-y-3">
                  ...
                  <p className="text-sm text-[#a1a1aa] leading-relaxed">
                    Здесь можно только <strong className="text-zinc-300 font-medium">проверить SMTP</strong> и{' '}
                    <strong className="text-zinc-300 font-medium">отправить одно тестовое письмо</strong>. План кампании,
                    правки текста, отбор и запуск массовой рассылки — в разделе «Рассылка по базе», чтобы всё было в
                    одном месте.
                  </p>
                  <Link
                    href="/outreach"
                    className="inline-flex items-center gap-2 rounded-lg bg-violet-600/90 hover:bg-violet-600 text-white text-sm font-semibold px-4 py-2.5"
                  >
                    Открыть «Рассылка по базе»
                    <FlaskConical className="w-4 h-4 opacity-90" />
                  </Link>
                </div>
              </>
            )}
