...
export function AIBrain() {
      ...
      try {
        ...
        if (r.ok) {
          ...
          setState({
            ...def,
            ...b,
            businessVertical: parseBusinessVertical(b.businessVertical),
  ...
  return (
    <CrmPage width="std" className="space-y-8 sm:space-y-10 pb-20 sm:pb-24">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="min-w-0">
          ...
          <p className="crm-page-lead max-w-2xl mt-3">
            ...
            нарушает ваши правила. Те же акции, промокоды, тон и рекомендованная частота касаний{' '}
            <strong className="text-zinc-300 font-medium">автоматически используются</strong> на странице{' '}
            <Link href="/outreach" className="text-sky-400 hover:text-sky-300 underline underline-offset-2">
              Рассылка по базе
            </Link>{' '}
            — после сохранения здесь нажмите «Обновить из настроек» там. Кнопки «Сгенерировать» и «Доработать с
            ИИ» ходят в API; модель Gemini задаётся{' '}
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <button
            type="button"
            onClick={handleSave}
            className={cn('crm-btn', 'crm-btn--primary', 'shadow-[0_0_20px_rgba(59,130,246,0.2)]')}
          >
      ...
      <div className="crm-card p-6 md:p-7 space-y-3 max-w-3xl">
        <h2 className="text-lg font-medium text-white">Сфера бизнеса</h2>
        <p className="text-sm text-[#71717a] leading-relaxed">
          От этого зависит список сценариев на странице «Автоматизация»: например, «брошенная корзина» только для
          интернет-магазина, а для салона — запись и пост-визит.
        </p>
        <NativeSelect
          variant="field"
          className="w-full max-w-md"
          value={state.businessVertical ?? 'mixed'}
          onChange={(e) =>
            setState((s) => ({
              ...s,
              businessVertical: parseBusinessVertical(e.target.value),
            }))
          }
          aria-label="Ниша бизнеса"
        >
        ...
        <p className="text-sm text-[#a1a1aa] leading-relaxed">
          {BUSINESS_VERTICAL_OPTIONS.find((x) => x.id === (state.businessVertical ?? 'mixed'))?.hint}
        </p>
        <p className="text-xs text-[#52525b] leading-relaxed border-t border-[#1f1f22] pt-3">
          <span className="font-semibold text-[#71717a] uppercase tracking-wide">Примеры бизнеса: </span>
          {BUSINESS_VERTICAL_OPTIONS.find((x) => x.id === (state.businessVertical ?? 'mixed'))?.examples}
        </p>
      </div>
